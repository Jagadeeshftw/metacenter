...
A live example any vault can copy is on mainnet: [coverage-guard-cached](/docs/contracts/coverage-guard-cached) reads `coverage-cache` through the trait and answers `ok` or `paused`. The same pattern, adapted to headroom:
